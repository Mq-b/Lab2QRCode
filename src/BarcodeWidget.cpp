...
BarcodeWidget::BarcodeWidget(QWidget* parent) : QWidget(parent) {
    ...
    barcodeLabel->setStyleSheet(
        "QLabel { background-color: #fafafa; padding: 16px; font-family: Consolas; font-size: 14pt;  }");
...
void BarcodeWidget::onBrowseFile() const { fileDialog->open(); }
...
void BarcodeWidget::onGenerateClicked() {
    ...
    connect(watcher, &QFutureWatcher<QPair<QImage, QString>>::finished, [this, watcher, reqWidth, reqHeight]() {
        ...
        if (const QString& errorMsg = result.second; !errorMsg.isEmpty()) {
            ...
            barcodeLabel->setPixmap(QPixmap::fromImage(lastImage).scaled(reqWidth, reqHeight, Qt::KeepAspectRatio,
    ...
    QFuture<QPair<QImage, QString>> future =
        QtConcurrent::run([filePath, useBase64, format, reqWidth, reqHeight]() -> QPair<QImage, QString> {
            try {
                QFile file(filePath);
...
void BarcodeWidget::onDecodeToChemFileClicked() {
    ...
    try {
        ...
        if (decodedData.size() > 1024)
            ...
            preview = QString::fromUtf8(reinterpret_cast<const char*>(decodedData.data()),
...
void BarcodeWidget::onSaveClicked() {
    ...
    const auto saveImage = [this](const QString& fileName) {
        ...
        const QFuture<bool> future = QtConcurrent::run([this, fileName] { return lastImage.save(fileName); });
        watcher->setFuture(future);
    };
    ...
    if (suffix == "png") {
        ...
        const QString savePath = QFileDialog::getSaveFileName(this, "保存文件", fileInfo.dir().filePath(defaultName),
        ...
        const QString fileName =
            QFileDialog::getSaveFileName(this, "保存图片", fileNameWithoutExtension + ".png", "PNG Images (*.png)");
...
QString BarcodeWidget::barcodeFormatToString(ZXing::BarcodeFormat format) {
    static const QMap<ZXing::BarcodeFormat, QString> map = {{ZXing::BarcodeFormat::None, "None"},
...
ZXing::BarcodeFormat BarcodeWidget::stringToBarcodeFormat(const QString& formatStr) {
    static const QMap<QString, ZXing::BarcodeFormat> map = {{"Aztec", ZXing::BarcodeFormat::Aztec},
